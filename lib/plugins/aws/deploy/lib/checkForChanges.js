...
module.exports = {
    ...
    return (
      this.provider
        ...
        .then((response) => {
          ...
          const oldStackName = this.getStackNameFromFilterName(filterName);
          const newStackName = this.provider.naming.getStackName();
          ...
          // If the old filter is not included in the new stack, just return
  ...
  getStackNameFromFilterName(filterName) {
    // See the comments of getLogicalIdFromFilterName to know the format of filter name
    const split = filterName.split('-');
